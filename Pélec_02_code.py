...
for i in BestParametresAB['AdaBoostRegressor()'][BestParametresAB['paramètre']
    ...
    fig3.show()
    if write_data is True:
        fig3.write_image('./Figures/graphRMSEAB{}.pdf'.format(i))
...
fig3.show()
if write_data is True:
    fig3.write_image('./Figures/graphRMSERidge_log.pdf')
...
fig3.show()
if write_data is True:
    fig3.write_image('./Figures/graphRMSELasso_log.pdf')
...
for i in BestParametresEN_log['ElasticNet()'][BestParametresEN_log['paramètre']
    ...
    fig3.show()
    if write_data is True:
        fig3.write_image('./Figures/graphRMSEEN_log{:.2}.pdf'.format(i))
...
# modèle kNN
# réglage des paramètre pour la gridsearch
n_neighbors_log = np.linspace(0, 100, dtype=int)
...
figkNN_log = reg_modelGrid(model=KNeighborsRegressor(n_jobs=-1),
                         scaler=scaler,
                         X_train=BEBM_train,
                         X_test=BEBM_test,
                         y_train=SiteEnergyUse_train_log,
...
fig3.show()
if write_data is True:
    fig3.write_image('./Figures/graphRMSEkNN_log.pdf')
...
for i in BestParametresRF_log['RandomForestRegressor()'][
    ...
    fig3.show()
    if write_data is True:
        fig3.write_image('./Figures/graphRMSERF_log{}.pdf'.format(i))
